...
from pre_processing.vod_data_conversion import concat_box_attr
from geometry import PolyArea2D_s, yaw_punish_factor, mask_between_boxes, \
    logical_or_mask, loop_convex, loop_inter, VodBox


def iou_bev_s(box_a: VodBox, box_b: VodBox) -> float:
    """
...
def iou_3d_s(box_a: VodBox, box_b: VodBox) -> Tuple[float, float]:
    """
...
def giou_bev_s(box_a: VodBox, box_b: VodBox) -> float:
    """
...
def giou_3d_s(box_a: VodBox, box_b: VodBox) -> Tuple[float, float]:
    """
...
def d_eucl_s(box_a: VodBox, box_b: VodBox) -> float:
    """
    Serial implementation of Euclidean Distance with yaw angle punish
    :param box_a: VodBox
    :param box_b: VodBox
    :return: Eucl distance between two VodBox
    """
    if box_b.name != box_a.name:
        return np.inf
