...
class UnscentedKalmanFilter(KalmanFilter):
    """
    Unscented Kalman Filter (UKF) for nonlinear motion models (CTRV / CTRA / BICYCLE).
    Interface/style is aligned with LinearKalmanFilter / ExtendKalmanFilter in this file.
    """

    def __init__(self, timestamp: int, config: dict, track_id: int, det_infos: dict) -> None:
        super().__init__(timestamp, config, track_id, det_infos)

        # motion model selection: prefer nonlinear models, otherwise fallback to CTRA
        ...
        # UKF hyper-parameters (can be moved to config if you want)
        ukf_cfg = config.get('ukf', {})
        self.alpha = float(ukf_cfg.get('alpha', 1e-3))
        self.beta = float(ukf_cfg.get('beta', 2.0))
        self.kappa = float(ukf_cfg.get('kappa', 0.0))
    ...
    def initialize(self, det_infos: dict) -> None:
        ...
        # init covariance (category-specific) + noises (fixed)
        self.P = self.model.getInitCovP(self.class_label)
        self.Q = self.model.getProcessNoiseQ()
        self.R = self.model.getMeaNoiseR()
    ...
    # UKF core math
    # --------------------------
    def _ukf_weights(self):
        n = self.SD
        lam = (self.alpha ** 2) * (n + self.kappa) - n

        Wm = np.zeros((2 * n + 1, 1), dtype=np.float64)
        Wc = np.zeros((2 * n + 1, 1), dtype=np.float64)

        Wm[0, 0] = lam / (n + lam)
        Wc[0, 0] = lam / (n + lam) + (1 - self.alpha ** 2 + self.beta)
    ...
    def _sigma_points(self, x: np.mat, P: np.mat):
        """
        Sigma points: X = [x, x ± sqrt((n+λ)P)]
        Returns:
            Xsig: np.mat shape (n, 2n+1)
        """
        n = self.SD
        lam, _, _ = self._ukf_weights()
        c = n + lam

        # Numerical stability: ensure symmetry
        P = (P + P.T) * 0.5

        # Cholesky (fallback to eigen if needed)
        try:
            A = np.linalg.cholesky((c * P).astype(np.float64))
        except np.linalg.LinAlgError:
            # small jitter for PSD
            jitter = 1e-9 * np.eye(n)
            A = np.linalg.cholesky((c * (P + jitter)).astype(np.float64))

        Xsig = np.mat(np.zeros((n, 2 * n + 1), dtype=np.float64))
        Xsig[:, 0] = x

        for i in range(n):
            col = np.mat(A[:, i]).reshape(n, 1)
            Xsig[:, 1 + i] = x + col
            Xsig[:, 1 + n + i] = x - col
        return Xsig

    def _unscented_transform(self, Xsig: np.mat, noise_cov: np.mat, nonlinear_fn):
        """
        Pass sigma points through nonlinear_fn, compute mean/cov.
        nonlinear_fn: f(x)->x'  OR h(x)->z
        Returns:
            x_mean: np.mat (dim,1)
            P_cov: np.mat (dim,dim)
            Ysig: np.mat (dim, 2n+1) transformed sigma points
        """
        _, Wm, Wc = self._ukf_weights()
        n_sig = Xsig.shape[1]

        # propagate sigma points
        y0 = nonlinear_fn(Xsig[:, 0])
        dim = int(y0.shape[0])

        Ysig = np.mat(np.zeros((dim, n_sig), dtype=np.float64))
        Ysig[:, 0] = y0
        for k in range(1, n_sig):
            Ysig[:, k] = nonlinear_fn(Xsig[:, k])

        # mean
        y_mean = np.mat(np.zeros((dim, 1), dtype=np.float64))
        for k in range(n_sig):
            y_mean += float(Wm[k, 0]) * Ysig[:, k]

        # covariance
        P = np.mat(np.zeros((dim, dim), dtype=np.float64))
        for k in range(n_sig):
            dy = Ysig[:, k] - y_mean
            P += float(Wc[k, 0]) * (dy * dy.T)
        P += noise_cov

        return y_mean, P, Ysig

    def predict(self, timestamp: int) -> None:
        # 1) sigma points from previous posterior
        Xsig = self._sigma_points(self.state, self.P)

        # 2) propagate through motion model f
        def f_fn(xk: np.mat) -> np.mat:
            return self.model.stateTransition(xk)

        x_pred, P_pred, Xsig_pred = self._unscented_transform(Xsig, self.Q, f_fn)
    ...
    def update(self, timestamp: int, det: dict = None) -> None:
        ...
        z = self.getMeasureInfo(det)  # [mz,1]

        # 1) sigma points from predicted state
        Xsig = self._sigma_points(self.state, self.P)

        # 2) measurement transform h
        def h_fn(xk: np.mat) -> np.mat:
            return self.model.StateToMeasure(xk)

        z_pred, S, Zsig = self._unscented_transform(Xsig, self.R, h_fn)

        # 3) cross covariance Pxz
        _, _, Wc = self._ukf_weights()
        n_sig = Xsig.shape[1]
        Pxz = np.mat(np.zeros((self.SD, z.shape[0]), dtype=np.float64))

        for k in range(n_sig):
            dx = Xsig[:, k] - self.state
            dz = Zsig[:, k] - z_pred
            Pxz += float(Wc[k, 0]) * (dx * dz.T)

        # 4) Kalman gain, innovation, update
        K = Pxz * S.I
        res = z - z_pred
        self.model.warpResYawToPi(res)

        self.state = self.state + K * res
