...
@HEADS.register_module()
class OccupancyHead(BaseModule):
    """OccupancyHead for CenterPoint.
    ...
    Args:
        ...
        share_occ_channel (int, optional): Output channels for share_conv
            layer. Default: 64.
        num_heatmap_occs (int, optional): Number of conv layers for heatmap
            conv layer. Default: 2.
        occ_cfg (dict, optional): Config of conv layer.
            Default: dict(type='Conv2d')
        norm_cfg (dict, optional): Config of norm layer.
            Default: dict(type='BN2d').
        bias (str, optional): Type of bias. Default: 'auto'.
    ...
    def __init__(self,
                 ...
                 separate_head=dict(
                     type='SeparateHead', init_bias=-2.19, final_kernel=3),
                 share_occ_pred_channel=64,
                 num_heatmap_occs=2,
                 occ_cfg=dict(type='Conv2d'),
                 norm_cfg=dict(type='BN2d'),
                 bias='auto',
                 norm_bbox=True,
                 init_cfg=None):
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(OccupancyHead, self).__init__(init_cfg=init_cfg)
        ...
        # a shared occupancy prediction network
        self.shared_occ_pred = ConvModule(
            in_channels,
            share_occ_pred_channel,
            kernel_size=3,
            padding=1,
            occ_cfg=occ_cfg,
            norm_cfg=norm_cfg,
        ...
        for num_cls in num_classes:
            ...
            heads.update(dict(heatmap=(num_cls, num_heatmap_occs)))
            separate_head.update(
                in_channels=share_occ_pred_channel, heads=heads, num_cls=num_cls)
